...
function dogHead(points_transform) {
	shape.call(this);
	this.populate(this, points_transform);
	if( !arguments.length) return; 
	this.init_buffers();
} inherit(dogHead, shape);

	dogHead.prototype.populate = function (recipient, points_transform){
		var index_offset = recipient.indices.length;

		recipient.vertices = [vec3(-1,0,0), vec3(1,0,0), vec3(0,-1,0), // nose
							vec3(0,0,0), vec3(-1.5,1,-3), vec3(0,1,-3),    // center snout
							vec3(0,1,-3),  vec3(-1.5,2.5,-4), vec3(0,2.5,-4),   // lowerforehead
							vec3(-1,0,0), vec3(-1.5,1,-3), vec3(0,0,0),   // nose corner to back snout
							vec3(-1,0,0), vec3(-1,-2,-1), vec3(0,-1,0), // nose corner to front lip
							vec3(-1,0,0), vec3(-1.5,1,-3), vec3(-1.5,-2,-3),    // nose corner-back snout-back-lip
							vec3(-1.5,-2,-3), vec3(-3,0,-6), vec3(-2,-2,-8), // back cheek-back jaw
							vec3(-1,0,0), vec3(-1.5,-2,-3),vec3(-1,-2,-1),  // nose corner to back mouth
							vec3(0,-1,0), vec3(-1.5,-2,-3), vec3(0,-2,-3), // roof of mouth 
							vec3(0,-1,0), vec3(-1.5,-2,-3), vec3(-1,-2,-1),  // lips
							vec3(-1.5,-2,-3), vec3(-1.5,1,-3), vec3(-2,0,-4), // front cheek
							vec3(-1.5,2.5,-4), vec3(0,1,-3), vec3(-1.5,1,-3),  // lower forehead to back snout
							vec3(-1.5,2.5,-4), vec3(0,2.5,-7), vec3(0,2.5,-4),  // middle top head
							vec3(-2,0,-4), vec3(-3,0,-6), vec3(-1.5,-2,-3),  // back cheek
							 vec3(-1.5,1,-3), vec3(-3,0,-6), vec3(-2,0,-4),     // back snout to back cheek
							vec3(-1.5,2.5,-4), vec3(-3,0,-6), vec3(-1.5,1,-3),  // lower forehead to back cheek
							vec3(-1.5,2.5,-4), vec3(-1.5,2.5,-7), vec3(0,2.5,-7),  // sides top head
							vec3(-1.5,2.5,-7), vec3(-2.5,2,-7), vec3(-2,4.5,-7), // ears
							vec3(-2.5,2,-7), vec3(-2,4.5,-7), vec3(-2,2,-8), // ear top-ear corner-ear back (outer)
							vec3(-2,4.5,-7), vec3(-2,2,-8), vec3(-1.5,2.5,-7), // ear top- ear corner-ear back (inner)
							vec3(-2.5,2,-7), vec3(-1.5,2.5,-7), vec3(-1.5,2.5,-4),  // ear corner to top of lower forehead 
							vec3(-2.5,2,-7),  vec3(-3,0,-6),vec3(-1.5,2.5,-4), // ear corner-back cheek-top lower forehead
							vec3(-2.5,2,-8), vec3(-3,0,-6), vec3(-2,4.5,-7), // back ear-back cheek
							 vec3(-3,0,-6),vec3(-2.5,2,-8), vec3(-2,-2,-8), // back ear-back cheek-back jaw
							 vec3(-2,2,-8),  vec3(0,2,-8), vec3(-1.5,2.5,-7),  // back ear (inner)-back head center
							 vec3(-1.5,2.5,-7), vec3(0,2,-8), vec3(0,2.5,-7) // back head center
							];

		recipient.texture_coords = [ vec2(0,0), vec2(0,1), vec2(1,0)];

		for (var i=0; i<recipient.vertices.length; i++)
		{
...
function dogMouth(points_transform) {
	shape.call(this);
	this.populate(this, points_transform);
	if( !arguments.length) return; 
	this.init_buffers();
} inherit(dogMouth, shape);

	dogMouth.prototype.populate = function (recipient, points_transform){
		var index_offset = recipient.indices.length;
		recipient.vertices = [ vec3(0,-2,-8), vec3(-1,-2,-1), vec3(0,-2,-1), // front mouth top
								vec3(0,-2,-1), vec3(-1,-2,-1), vec3(0,-3,-2),  // front mouth bottom
								vec3(0,-3,-2), vec3(-1.5,-2,-3), vec3(-2,-2,-5),   // front mouth bottom to back jaw
								vec3(0,-3,-2), vec3(-1,-2,-1), vec3(-1.5,-2,-3),  // snout to chin 
								vec3(0,-3,-2), vec3(0,-2,-5), vec3(-2,-2,-5)  // chin to back jaw (middle)
								];
		recipient.texture_coords = [ vec2(0,0), vec2(0,1), vec2(1,0) ];
...
function dogBody(points_transform) {
	shape.call(this);
	this.populate(this, points_transform);
	if( !arguments.length) return; 
	this.init_buffers();
} inherit(dogBody, shape);

	dogBody.prototype.populate = function (recipient, points_transform, c){
		var index_offset = recipient.indices.length;
		recipient.vertices = [ vec3(-2.5,2,0), vec3(-2,-2,-4), vec3(-2,-2,0),  // ear-topneck
								vec3(-2,-2,0), vec3(-2,-10,-1), vec3(0,-2,3), // under jaw-bottom neck
								vec3(-2,-10,-1),vec3(0,-10,-1), vec3(0,-2,3), // bottom neck center
								vec3(-2.5,2,0), vec3(0,2,0), vec3(-2,-2,-4), // behind ear-topneck
								vec3(0,2,0), vec3(0,-2,-4), vec3(-2,-2,-4), // neck center back
								vec3(-2,-2,-4), vec3(0,-2,-4), vec3(-2,-2,-15),  // neck center-butt (body length) outer triangles
								vec3(0,-2,-4), vec3(0,-2,-15), vec3(-2,-2,-15), // neck ceneter-butt (body length) center triangles
								vec3(-2,-10,-1), vec3(-2,-2,0), vec3(-2,-2,-15), // body length (top)
								 vec3(-2,-8,-15), vec3(-2,-10,-1), vec3(-2,-2,-15),  // body length (bottom)
								vec3(-2,-8,-15), vec3(-2,-2,-15), vec3(-2,-7,-20),   // butt side
								vec3(-2,-7,-20), vec3(-2,-2,-15), vec3(0,-7,-20),  // butt cheek outer 
								vec3(0,-2,-15), vec3(0,-7,-20), vec3(-2,-2,-15), // butt cheek inner
								vec3(-2,-7,-20), vec3(0,-7,-20), vec3(-2,-10,-1), // belly sides
								vec3(0,-7,-20), vec3(-2,-10,-1), vec3(0,-10,-1) // belly center
								];
		recipient.texture_coords = [ vec2(0,0), vec2(0,1), vec2(1,0) ];
	...
	function dogShoulder(points_transform) {
	shape.call(this);
	this.populate(this, points_transform);
	if( !arguments.length) return; 
	this.init_buffers();
} inherit(dogShoulder, shape);

	dogShoulder.prototype.populate = function (recipient, points_transform, c){
		var index_offset = recipient.indices.length;
		recipient.vertices = [vec3(-1,0,0), vec3(0,3,2), vec3(0,1.5,-4), // top shoulder
								vec3(-1,0,0), vec3(0,1.5,-4), vec3(0,-3,-5),  // back shoulder
								vec3(-1,0,0), vec3(0,-1,2), vec3(0,3,2),  // front shoulder (top)
								vec3(-1,0,0), vec3(0,-3,-1), vec3(0,-1,2),  // front shoulder (bottom)
								vec3(-1,0,0), vec3(0,-3,-5),vec3(0,-3,-1), // bottom shoulder

								vec3(0,-3,-1), vec3(1.5,-3,-1), vec3(0,-1,2), // front shoulder to inner body
								vec3(0,1.5,-4), vec3(0,-3,-5), vec3(1.5,-3,-5), // back shoulder to inner body
								vec3(1.5,-3,-1), vec3(0,1.5,-4), vec3(0,-1,2), // inner shoulder
								vec3(1.5,-3,-1), vec3(1.5,-3,-5), vec3(0,1.5,-4) // inner shoulder
								];
		recipient.texture_coords = [ vec2(0,0), vec2(0,1), vec2(1,0) ];
...
function dogForearm(points_transform) {
	shape.call(this);
	this.populate(this, points_transform);
	if( !arguments.length) return; 
	this.init_buffers();
} inherit(dogForearm, shape);

	dogForearm.prototype.populate = function (recipient, points_transform){
		var index_offset = recipient.indices.length;
		recipient.vertices = [vec3(0,0,-4), vec3(1,-10,-3), vec3(0,0,0),  // outer forearm
								vec3(1.5,0,0), vec3(1,-10,-3), vec3(1.5,0,-4), // inner forearm
								vec3(0,0,0), vec3(1.5,0,-3), vec3(1,-10,-3), // front forearm
								vec3(0,0,-4), vec3(1.5,0,-4), vec3(1,-10,-3) // back forearm
								];
		recipient.texture_coords = [ vec2(0,0), vec2(0,1), vec2(1,0) ];
	...
	function dogTail(points_transform) {
	shape.call(this);
	this.populate(this, points_transform);
	if( !arguments.length) return; 
	this.init_buffers();
} inherit(dogTail, shape);

	dogTail.prototype.populate = function (recipient, points_transform){
		var index_offset = recipient.indices.length;
		recipient.vertices = [vec3(-2,0,0), vec3(0,3,-4), vec3(0,0,0), // front tail seg 1
							vec3(-2,0,0), vec3(0,3,-4), vec3(0,-1.5,-1.5), // right side tail seg 1
							vec3(0,3,-4), vec3(-1,1.5,-2), vec3(0,5,-2), // right side tail seg 2
							vec3(-1,1.5,-2), vec3(0,1.5,-2), vec3(0,5,-2), // front tail seg 2
							vec3(0,5,-2), vec3(-0.7,2.5,-2), vec3(0,2.5,0) // right side tail seg 3
...
function dogPoo(points_transform) {
	shape.call(this);
	this.populate(this, points_transform);
	if( !arguments.length) return; 
	this.init_buffers();
} inherit(dogPoo, shape);

	dogPoo.prototype.populate = function (recipient, points_transform){
		var index_offset = recipient.indices.length;
		recipient.vertices = [vec3(0,1,0), vec3(-1,0,0), vec3(1,0,0),
								vec3(0,1,-5), vec3(-1,0,-5), vec3(1,0,-5)
		...
		recipient.normals = [vec3(1,1,1), vec3(-1,-1,-1), vec3(1,0,1), vec3(-1,0,-1),
